...
internal class FolderUpgrader
{
    ...
    public void SetThresholds(double? jaccardThreshold = null, double? levenshteinThreshold = null,
        int? minLength = null)
    {
        if (jaccardThreshold.HasValue) JaccardThreshold = jaccardThreshold.Value;
        if (levenshteinThreshold.HasValue) LevenshteinThreshold = levenshteinThreshold.Value;
    ...
    public void UpgradeFolders(string originalRootPath)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            if (string.IsNullOrEmpty(currentPath) || !Directory.Exists(director)) continue;

            foreach (string subDirectory in Directory.GetDirectories(director)) stack.Push(subDirectory);

            if (UpgradeSingleFolder(currentPath))
            {
                string? parentPath = Path.GetDirectoryName(currentPath);
                if (!string.IsNullOrEmpty(parentPath) && currentPath != originalRootPath) stack.Push(parentPath);
            }
        }
    }
    ...
    private bool UpgradeSingleFolder(string folderPath)
    {
        ...
        if (!IsSimilarEnough(Path.GetFileName(folderPath), Path.GetFileName(subFolderPath))) return false;

        if (MoveContentsToParent(subFolderPath, folderPath)) DeleteEmptyFolders(subFolderPath, folderPath);
        return true;
    }
    ...
    private static void DeleteEmptyFolders(string currentFolder, string stopFolder)
    {
        ...
        {
            if (!Directory.Exists(currentFolder)) break;
    ...
    private bool IsSimilarEnough(string str1, string str2)
    {
        ...
        if (jaccardSimilarity >= JaccardThreshold && str1.Length > 5 && str2.Length > 5) return true;

        if (str1.Length <= MinLength || str2.Length <= MinLength) return str1 == str2;

        double levenshteinSimilarity =
            CalculateLevenshteinSimilarity(string.Join(" ", parts1), string.Join(" ", parts2));
